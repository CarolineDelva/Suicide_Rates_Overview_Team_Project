...
//   const isGeneration = name => allCaps(name) && name !== 'WORLD';

  const melt = (Data, minYear, maxYear) => {
    const years = d3.range(minYear, maxYear + 1);

    const data = [];

    // unData.forEach(d => {
    //   const name = d['generation*']
    //     .replace('AND THE', '&');
    //   years.forEach(year => {
    ...
    return data.filter(d => isGeneration(d.name));
  };

  const loadAndProcessData = () => 
    Promise
      .all([
        d3.json("/suicideratesdata"),
      ])
    //   .then(([unDataSuicdie_Rates]) => {
    //     return melt(unDataEstimates, 1985, 2016)
    //       .concat(melt(unDataMediumVariant, 2010, 2016));
    //   });
  ...
  const render = () => {
    ...
    lineChartG.call(lineChart, {
      ...
      colorValue,
      yValue,
      title: 'Suicide Rates Over Time Per Generation',
      xValue: d => d.year,
      xAxisLabel: 'Year',
      circleRadius: 6,
      yAxisLabel: 'Suicide Number',
      margin: {
        top: 60,
        right: 280,
        bottom: 88,
